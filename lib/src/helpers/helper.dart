...
class Helper {
  ...
  static Future<Marker> getMarker(Map<String, dynamic> res) async {
    final Uint8List markerIcon = res['id'] == "-1" ?
        await getBytesFromAsset('assets/img/driver_marker.png', 120)
...
//  }
  ...
  static Widget getWaitingTime(int waitingTime, BuildContext context,
      {TextStyle style}) {
    if (style != null) {
  ...
  static double getTotalOrderPrice(Order order) {
    double total = 0.0;
    ...
    total += order.tax * total / 100 + order.deliveryFee;
    return total;
  }

  static double getFoodPrice(FoodOrder foodOrder) {
