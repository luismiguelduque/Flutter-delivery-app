...
class DeliveryAddressesController extends ControllerMVC {
  ...
  Future<void> updateAddress(model.Address address) async {
    ...
    List<Placemark> placemark =
        await Geolocator().placemarkFromAddress(address.address);
    if (placemark.length > 0) {
      address.latitude = placemark.elementAt(0).position.latitude.toString();
      address.longitude = placemark.elementAt(0).position.longitude.toString();
    } else {
      setState(() {});
      return;
    }
